...
Page({
            ...
            if (res.data.resObject) {
              ...
              _this.setData({
                ...
                /*  // // 排列锚点-----分割线--------------分割线--------------分割线--------------分割线--------------
                    // var inf = _this.data.card_list[0].mycard_initial[0];
                    // var arr = [];
  ...
  getWait(_this,userid){
    wx.request({
          ...
          method: 'POST',
          success: function (res) {
            console.log('获取待接收列表成功');
            console.log(res)
            if (res.data.resObject) {
              console.log('有');
              _this.setData({
                waitNum: res.data.resObject.length
              });
              wx.stopPullDownRefresh();
            } else {
              console.log('还没有待接收名片');
              _this.setData({waitNum: 0});
            }
          },
          fail: function (res) {
            wx.stopPullDownRefresh();
  ...
  onShareAppMessage(res) {
    ...
    if (res.target) {
      ...
      return {
        title: '版图-最具人气的名片交互!',
        desc: '最具人气的名片交互!',
        path: '/pages/card_Dls/card_Dls?cardid=' + res.target.dataset.card + '&userId=' + that.data.userid,
